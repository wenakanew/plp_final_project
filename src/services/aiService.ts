...
export const generateAnyContent = async (prompt: string, contentType: string): Promise<ContentGenerationResult | null> => {
  // Check if Azure OpenAI is configured
  if (!apiConfig.azureOpenAI.isEnabled) {
    console.log("Azure OpenAI is not enabled, using mock content generation");
    return mockContentGeneration(prompt, contentType);
  }

  try {
    // Prepare system prompt based on content type
    const systemPrompt = `You are a professional content creator specializing in ${contentType}.
Create high-quality, engaging content based on the user's request.
Follow these guidelines:
1. Create a compelling title (first line of the response)
2. Write well-structured content with clear sections
3. Use appropriate tone and style for ${contentType}
4. Include relevant details and examples
5. Ensure factual accuracy and coherence
6. Format the content with proper paragraphs and sections`;
    ...
    toast.error("There was an error generating content. Using simplified version instead.");
    return mockContentGeneration(prompt, contentType);
  }
};
...
// Mock content generation for when AI is not available
const mockContentGeneration = (prompt: string, contentType: string): ContentGenerationResult => {
  const today = new Date().toLocaleDateString('en-US', { 
    year: 'numeric', 
    month: 'long', 
    day: 'numeric' 
  ...
  switch (contentType.toLowerCase()) {
    case 'article':
      title = `Sample Article: ${prompt}`;
      content = `${title}\n\nThis is a sample article about ${prompt}. The content would be generated by AI in a production environment.\n\nKey points about ${prompt}:\n\n1. First important aspect\n2. Second important aspect\n3. Third important aspect\n\nConclusion: This is a placeholder for AI-generated content about ${prompt}.`;
      break;
      
    case 'story':
      title = `A Story About ${prompt}`;
      content = `${title}\n\nOnce upon a time, there was a story about ${prompt}. This is a placeholder for an AI-generated story.\n\nThe story would include:\n\n- An engaging beginning\n- An interesting middle\n- A satisfying conclusion\n\nThis is where the AI would create an engaging narrative about ${prompt}.`;
      break;
      
    case 'poem':
      title = `Ode to ${prompt}`;
      content = `${title}\n\nRoses are red,\nViolets are blue,\nThis is a placeholder\nFor a poem about ${prompt}.\n\nIn a real implementation,\nThe AI would create\nA beautiful poem\nThat's truly great.`;
      break;
      
    default:
      title = `Content About ${prompt}`;
      content = `${title}\n\nThis is a placeholder for AI-generated content about ${prompt}. The content would be tailored to the specific type requested (${contentType}).\n\nIn a production environment, this would be replaced with high-quality, AI-generated content that matches the requested style and format.`;
  }
  
  return {
    content,
    title,
    topics: [prompt.toLowerCase(), contentType.toLowerCase()]
  };
};
